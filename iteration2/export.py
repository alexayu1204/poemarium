...
INTERACTION_EVENTS = (
    ...
    "mediation_requested", "mediation_completed", "mediation_failed",
)
MEDIATION_EVENTS = ("mediation_requested", "mediation_completed", "mediation_failed")
...
def session_rows(rows: list[dict], session_id: str) -> list[dict]:
    ...
    v3 = [r for r in mine if (r.get("schema_version") or 0) >= 3]
    if not v3:
...
def _item_ids(row: dict) -> list:

    ids = row.get("item_ids")
    if ids:
        return list(ids)
    return [row["item_id"]] if row.get("item_id") else []
...
def reconstruct_episodes(all_rows: list[dict], items_by_id: dict) -> dict:
    ...
    mediations: dict[str, dict] = {}
    for r in rows:
        ev = r.get("event")
        if ev == "mediation_requested":
            mediations[r.get("mediation_id")] = {
                "mediation_id": r.get("mediation_id"), "role": r.get("role"),
                "consult_id": r.get("consult_id"), "item_id": r.get("item_id"),
                "item_ids": _item_ids(r),
                "line": r.get("line"), "line_index": r.get("line_index"),
                "poem_version": r.get("poem_version"), "poem_hash": r.get("poem_hash"),
                "requested_ts": r.get("ts"),
                "provider": r.get("provider"), "model": r.get("model"),
                "decoding": r.get("decoding"), "inputs": r.get("inputs"),
                "recurrence_supplied": r.get("recurrence_supplied"),
                "recurrence_context": r.get("recurrence_context"),
                "other_output_supplied": r.get("other_output_supplied"),
                "other_mediation_id": r.get("other_mediation_id"),
                "status": "requested", "output": None, "error": None,
                "latency_ms": None, "usage": None, "response_id": None,
            }
        elif ev in ("mediation_completed", "mediation_failed"):
            m = mediations.setdefault(r.get("mediation_id"), {
                "mediation_id": r.get("mediation_id"), "role": r.get("role"),
                "consult_id": r.get("consult_id"), "item_id": r.get("item_id"),
                "item_ids": _item_ids(r), "status": "requested"})
            m["status"] = "completed" if ev == "mediation_completed" else "failed"
            m["completed_ts"] = r.get("ts")
            m["output"] = r.get("output")
            m["error"] = r.get("error")
            m["latency_ms"] = r.get("latency_ms")
            m["usage"] = r.get("usage")
            m["response_id"] = r.get("response_id")
            m["model_reported"] = r.get("model")
    reflection_row = next((r for r in reversed(rows) if r.get("event") == "session_reflection"), None)
    ...
    for n, c in enumerate(consults):
        ...
        episodes.append({
            ...
            "mediations": [m for m in mediations.values() if m.get("consult_id") == c["consult_id"]],
            "diff_to_after": diff,
        })
    ...
    return {
        ...
        "schema_version": max(r.get("schema_version") or 0 for r in rows),
        ...
        "counts": {
            ...
            "mediations_completed": sum(1 for m in mediations.values() if m["status"] == "completed"),
            "mediations_failed": sum(1 for m in mediations.values() if m["status"] == "failed"),
        },
        ...
        "mediations": list(mediations.values()),
        "session_reflection": ({"schema": reflection_row.get("schema"),
                                "answers": reflection_row.get("answers"),
                                "ts": reflection_row.get("ts")} if reflection_row else None),
        "mediation_condition": (start or {}).get("mediation_condition"),
        "phase": (start or {}).get("phase"),
...
def render_episodes_md(data: dict, items_by_id: dict | None = None) -> str:
    ...
    o.append(f"- consults {c['consults']} (discarded {c['consults_discarded']}, failed "
             ...
             f"cycle reflections {c['reflections']} · mediations {c['mediations']} "
             f"(completed {c['mediations_completed']}, failed {c['mediations_failed']})")
    if data.get("mediation_condition"):
        mc = data["mediation_condition"]
        o.append(f"- mediation condition: {mc.get('provider')} · {mc.get('model')} · "
                 f"decoding {mc.get('decoding')} · {mc.get('phase_note') or ''} · phase {data.get('phase')}")
    inh = data["inherited"]
    ...
    for ep in data["episodes"]:
        ...
        if ep["notes"]:
            ...
            for n in ep["notes"]:
                o.append(f"- [{n.get('action')}] line {n.get('line_index')} “{n.get('line_text')}”: "
                         f"{n.get('text')}")
            o.append("")
        if ep.get("mediations"):
            o.append("### Mediation (explicitly invoked)")
            o.append("")
            for m in ep["mediations"]:
                ids = m.get("item_ids") or ([m["item_id"]] if m.get("item_id") else [])
                units = " + ".join(
                    "“" + ((items_by_id.get(i, {}).get("display_text") or i or "").split("\n")[0]) + "”"
                    for i in ids)
                head = (f"**{(m.get('role') or '?').upper()}** on {units} ({', '.join(ids)}) — {m.get('status')} · "
                        f"{m.get('provider')} {m.get('model')}")
                if m.get("other_output_supplied"):
                    head += " · given the Connect output"
                if m.get("recurrence_supplied"):
                    head += " · given recurrence context"
                o.append(head)
                o.append("")
                if m.get("status") == "completed":
                    o.append("> " + (m.get("output") or "").replace("\n", "\n> "))
                else:
                    o.append(f"> failed: {m.get('error')}")
                o.append("")
        ...
        o.append("")
    if data.get("session_reflection"):
        o += ["## Session reflection (structured)", ""]
        for a in data["session_reflection"].get("answers") or []:
            o.append(f"**{a.get('question')}**")
            o.append("")
            o.append(a.get("text") or "(no answer)")
            o.append("")
    if data["session_note"]:
        o += ["## Session note", "", data["session_note"], ""]
...
mediation.jsonl         Recorded mediation requests and results.
"""
...
def write_bundle(research_dir: Path, data_dir: Path, session_id: str) -> list[Path]:
    ...
    put("episodes.md", render_episodes_md(data, items))
    put("mediation.jsonl", "".join(json.dumps(r, ensure_ascii=False) + "\n"
                                   for r in mine if r.get("event") in MEDIATION_EVENTS))
...
def main(argv=None) -> int:
    ...
    targets = [args.session] if args.session else \
        [s["session_id"] for s in found if any(v >= 3 for v in s["schemas"])]
    rc = 0
    for sid in targets:
        try:
