...
from mediation import MediationError, ROLE_NAMES, make_provider
...
class MediateRequest(CleanModel):
    role: str
    consult_id: str
    item_id: str = ""
    item_ids: list[str] = []
...
class ReflectionAnswer(CleanModel):
    id: str
    question: str = ""
    text: str = ""


class SessionReflectionRequest(CleanModel):
    schema_id: str = "session-notes-v1"
...
def create_app(data_dir: str | Path | None = None,
               ...
               embedder=None, exclusive: bool = True, mediator=None) -> FastAPI:
    ...
    research_dir = Path(research_dir or os.environ.get("POETRY_RESEARCH_DIR")
                        or (BASE_DIR / "research" / config.STUDY_PHASE))
    if Path(research_dir).resolve() == (BASE_DIR / config.I1_RECORD_DIR).resolve():
        raise RuntimeError(
            f"{research_dir} is the Iteration 1 record; "
            "Iteration 2 never writes there")
    ...
    async def lifespan(app: FastAPI):
        ...
        st.mediator = mediator if mediator is not None else make_provider()
        st.phase = config.STUDY_PHASE
        ...
        if stored.get("session_id") and stored.get("ended_utc") is None:
            ...
            _no_session(stored.get("session_id") if stored.get("ended_utc") else None)
        ...
        if st.session is None:
            _log(app, "server_started", {"awaiting": "session start"})
        try:
            yield
        finally:
    ...
    def _no_session(last_ended_id: str | None) -> None:

        st = app.state
        st.session = None
        st.session_id = last_ended_id
        st.session_ended = bool(last_ended_id)
        st.session_resumed = False
        st.seen_client_rows = set()
        st.last_consult = None
        st.poem_version = None
        st.last_poem_hash = None

    def _require_session() -> None:
        st = app.state
        if st.session is None:
            raise HTTPException(409, "no session is open — start one first")
    ...
    def _begin_session(session: dict, draft: dict, pins: list,
        ...
        pins_cleared: list = []
        if not resumed and config.PINS_RESET_AT_SESSION_START and pins:


            pins_cleared = [p.get("item_id") for p in pins]
            _write_json(pins_path, [])
            pins = []
        ...
        if resumed:
            ...
            _log(app, "session_start", {
                ...
                "pins_cleared_at_start": pins_cleared,
                ...
                "phase": config.STUDY_PHASE,
                "mediation_condition": st.mediator.describe(),
                **extra,
            })
        if changed:
    ...
    def _session_view() -> dict | None:
        ...
        return {
            ...
            "mediations": list(s.get("mediations", {}).values()),
            "phase": s.get("phase", config.STUDY_PHASE),
        }
    ...
    async def state():
        ...
        return {
            ...
            "config": {
                ...
                "semantic_unavailable_reason": st.semantic_unavailable_reason,
                "embeddings_archive_hash": st.embeddings_archive_hash,
                "phase": st.phase,
                "mediation": {"provider": st.mediator.name,
                              "available": getattr(st.mediator, "available", True)},
                "pins_reset_at_session_start": config.PINS_RESET_AT_SESSION_START,
            },
        }
    ...
    async def event(req: EventRequest):
        ...
        if req.type == "session_end" and st.session is None:
    ...
    @app.post("/session_reflection")
    async def session_reflection(req: SessionReflectionRequest):

        _require_session()
        answers = [a.model_dump() for a in req.answers]
        _log(app, "session_reflection", {
            "schema": req.schema_id, "answers": answers, "client_ts": req.client_ts})
        return {"ok": True, "answered": sum(1 for a in answers if a["text"].strip())}
    ...
    async def _start_session(req: NewSessionRequest) -> dict:

        st = app.state
        if st.session is not None and not st.session_ended:
    ...
    async def session_start(req: NewSessionRequest):
    ...
    @app.get("/recurrence/{item_id}")
    async def recurrence_summary(item_id: str):

        st = app.state
        if item_id not in st.items_by_id:
            raise HTTPException(404, f"unknown item {item_id}")
        encs = recurrence_mod.encounters(item_id, st.session_id, log_path, st.phase)
        return recurrence_mod.summary(encs)

    @app.post("/mediate")
    async def mediate(req: MediateRequest):

        st = app.state
        _require_session()
        if st.session_ended:
            raise HTTPException(409, "the session has ended — mediation is closed with it")
        if req.role not in ROLE_NAMES:
            raise HTTPException(422, "role must be connect or reframe")
        if not getattr(st.mediator, "available", True):
            raise HTTPException(503, "No interpretation model is configured.")
        if req.consult_id not in st.session.get("consult_ids", []):
            raise HTTPException(422, "consult_id is not a consult of this session")
        if req.role == "connect":


            ids = list(req.item_ids) or ([req.item_id] if req.item_id else [])
            if not 1 <= len(ids) <= config.CONNECT_MAX_ITEMS:
                raise HTTPException(422, f"Connect takes one to {config.CONNECT_MAX_ITEMS} chosen items")
            if len(set(ids)) != len(ids):
                raise HTTPException(422, "the same item is chosen twice")
            missing = [i for i in ids if i not in st.items_by_id]
            if missing:
                raise HTTPException(404, f"unknown item {missing[0]}")
            item, items = None, [st.items_by_id[i] for i in ids]
        else:
            if not req.item_id:
                raise HTTPException(422, "Reframe needs the opened item")
            item = st.items_by_id.get(req.item_id)
            if item is None:
                raise HTTPException(404, f"unknown item {req.item_id}")
            ids, items = [req.item_id], None
        version, phash = _record_poem(req.title, req.draft_text, "mediation")
        meds = st.session.setdefault("mediations", {})
        mediation_id = f"m_{_compact(_utcnow())}_{len(meds) + 1:03d}"
        role_name = ROLE_NAMES[req.role]
        ...
        pins_now: list[dict] = []
        if req.role == "reframe":
            for m in reversed(list(meds.values())):
                if (m.get("role") == "connector" and m.get("consult_id") == req.consult_id
                        and req.item_id in (m.get("item_ids") or [m.get("item_id")])
                        and m.get("status") == "completed"):
                    other = m
                    break
            recurrence = recurrence_mod.encounters(req.item_id, st.session_id, log_path, st.phase) or None
            pins_now = [st.items_by_id[p["item_id"]] for p in _pins()
                        if p.get("item_id") in st.items_by_id]
        inputs = {
            "title": req.title,
            "poem_text": req.draft_text,
            "line": req.line,
            "line_index": req.line_index,
            "items": items if items is not None else [item],
            "pins": pins_now,
            "previous_output": other["output"] if other else None,
            "recurrence": recurrence,
        }
        desc = st.mediator.describe()
        common = {
            "mediation_id": mediation_id, "role": role_name, "action": req.role,
            "consult_id": req.consult_id,
            "item_id": req.item_id if req.role == "reframe" else None,
            "item_ids": ids,
            "line": req.line, "line_index": req.line_index,
        }
        _log(app, "mediation_requested", {
            **common,
            "poem_hash": phash,
            "provider": desc.get("provider"), "model": desc.get("model"),
            "decoding": desc.get("decoding"),
            "inputs": inputs,
            "pins_supplied": [p["item_id"] for p in pins_now],
            "recurrence_supplied": bool(recurrence),
            "recurrence_context": recurrence,
            "other_output_supplied": bool(other),
            "other_mediation_id": other["mediation_id"] if other else None,
            "client_ts": req.client_ts,
        })
        card = {**common, "status": "requested", "output": None, "error": None,
                "latency_ms": None, "recurrence_supplied": bool(recurrence),
                "other_output_supplied": bool(other), "poem_version": version}
        try:
            res = st.mediator.generate(req.role, inputs)
        except MediationError as exc:
            failure = (str(exc), exc.provider, exc.model, exc.latency_ms)
        except Exception as exc:
            failure = (f"{type(exc).__name__}: {str(exc)[:300]}", desc.get("provider"),
                       desc.get("model"), None)
        else:
            failure = None
        if failure is not None:
            error, prov, model, latency = failure
            card.update(status="failed", error=error, latency_ms=latency)
            meds[mediation_id] = card
            _save_session()
            _log(app, "mediation_failed", {
                **common, "error": error, "provider": prov, "model": model,
                "latency_ms": latency})
            return {"ok": True, "mediation": card}
        card.update(status="completed", output=res.text, latency_ms=res.latency_ms)
        meds[mediation_id] = card
        _save_session()
        _log(app, "mediation_completed", {
            **common, "output": res.text, "provider": res.provider, "model": res.model,
            "response_id": res.response_id, "usage": res.usage,
            "latency_ms": res.latency_ms, "finish": res.finish, "raw": res.raw})
        return {"ok": True, "mediation": card}
