...
function scheduleSave() {
  if (!draftLoaded || state.readOnlyConflict) return;
  draftDirty = true;
...
function tooltipShow(item, res, x, y) {
  ...
  const meta = [];
  if (typeof res.score === 'number') meta.push('cosine ' + res.score.toFixed(3));
  if (item) meta.push(typeWord(item.item_type));
  tooltip.appendChild(el('span', 'tooltip-meta micro', meta.join(' · ')));
...
const MAX_PERSIST = 7;
const LABEL_H = 13;
const PERSIST_CHARS = 28;
const PERSIST_UNITS = 190;
const HOVER_CHARS = 60;
const HOVER_UNITS = 240;
const CHAR_EST = 6.2;
const EDGE = 4;
const PAD = 2;
const GLYPH_OBS = 18;
const PEEK_MS = 300;
const FIELD_LAYOUT = 'field/v3';

const FIELD_CONSTANTS = {
  FIELD_W, GOLDEN, R_IN, R_OUT, G_MIN, EPS_SPAN, N_PERSIST, MAX_PERSIST,
  PERSIST_CHARS, HOVER_CHARS, PEEK_MS, field_layout: FIELD_LAYOUT,
  tooltip_cosine: true,
};
...
function findSlot(gx, gy, w, obstacles) {
  ...
  let firstInBounds = null;
  for (const gap of [10, 20]) {
    for (const d of dirs) {
      const c = candidateRect(gx, gy, w, d, gap);
      if (!inBounds(c)) continue;
      if (!firstInBounds) firstInBounds = c;
      if (!obstacles.some((o) => rectsOverlap(o, c))) return { rect: c, free: true };
    }
  }
  if (!firstInBounds) {
  ...
  return { rect: firstInBounds, free: false };
}
...
function renderField(resp) {
  ...
  for (const n of nodes) {
    if (persistent.size >= MAX_PERSIST) break;
    if (n.i < N_PERSIST || isPinned(n.res.item_id)) persistent.add(n.res.item_id);
  }
  ...
  const placed = [];
  for (const n of nodes) {
    if (!persistent.has(n.res.item_id)) continue;
    ...
    if (slot.free) {
      ...
      labelState.set(n.res.item_id, 'visible');
    } else {
      persistent.delete(n.res.item_id);
      labelState.set(n.res.item_id, 'demoted');
    }
  }
  ...
  for (const n of nodes) {
    ...
    const others = glyphRects.filter((_, j) => j !== n.i);
    const slot = findSlot(n.x, n.y, labels[n.i].width, obstacles.concat(others, placed));
    placement.set(n.res.item_id, { rect: slot.rect, hoverOnly: true });
    if (!labelState.has(n.res.item_id)) labelState.set(n.res.item_id, 'hover');
  }
  ...
  for (const n of nodes) {
    ...
    g.addEventListener('mouseenter', (e) => {
      if (p.hoverOnly && field.lastElementChild !== g) field.appendChild(g);
  ...
  return {
    ...
    label_policy: 'top' + N_PERSIST + '+pinned,cap' + MAX_PERSIST,
    nodes: nodes.map((n, i) => ({
...
async function finishSession(withNote) {
  ...
  try {
    ...
    await flushDraftSave();
    if (withNote && text && state.noteSentText !== text) {
      await apiPost('/session_note', { text });
      state.noteSentText = text;
    }
    ...
    $('#session-pill').classList.add('ended');
...
async function startNewSession(clearPoem) {
  try {
    await apiPost('/session/new', { clear_poem: clearPoem, client_ts: nowIso() });
...
function renderStatus(cfg) {
  ...
  if (s.ended_utc) {
    ...
    $('#new-session').hidden = false;
    $('#session-pill').classList.add('ended');
    $('#session-pill-text').textContent = 'Session ended';
  }
}
...
function claimWriter() {
  ...
  channel = new BroadcastChannel('poemarium-iteration1');
...
async function init() {
  ...
  try {
    ...
    draftLoaded = true;
    ta.readOnly = state.readOnlyConflict;
    titleEl.readOnly = state.readOnlyConflict;
    const cfg = st.config || {};
    renderStatus(cfg);
    setOnline();
    wasOffline = false;
